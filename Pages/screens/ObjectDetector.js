...
export default async function detectObjects(image) {
  ...
  if (model == null) {
    console.log('Loading model wait...');
    // const filePath = await MobileModel.download(MODEL_URL);
    // const mainBundlePath = FileSystem.documentDirectory;
    if (mainBundlePath) {
      // console.log('Main bundle path:', mainBundlePath);
      // const filePath = mainBundlePath + 'yolov5Model/best.pt';
      model = await torch.jit._loadForMobile('best.pt');
      console.log('Model successfully loaded');
    } else {
      console.log('Cannot load model. Main bundle path is undefined.');
    }

...
import { StyleSheet, View, Modal, Text, Image, ScrollView, AccessibilityInfo } from 'react-native';
...
export default function Barcode({ navigation }) {
  ...
  React.useLayoutEffect(() => {
    AccessibilityInfo.announceForAccessibility("인식중");
    ...
    if (barcodeResults !== undefined) {
      if (barcodeResults[0] !== undefined) {
        if (barcodeResults[0].barcodeText !== undefined) {
          AccessibilityInfo.announceForAccessibility("바코드가 인식 되었습니다 결과를 기달려 주세요");
            ...
            <View style={styles.modalView}>
              {nobar === true ? (
                <View accessible={true}>
                  ...
                  <TouchableRipple style={styles.button} onPress={() => { setModalVisible(false); setUseCamera(true); setNobar(!nobar); }} accessible={true} accessibilityLabel='닫기' accessibilityRole='button' accessibilityHint='바코드를 인식하러 돌아갑니다.'>
                ...
                <View>
                  ...
                  {datatype === "food" ? (
                    <View>
                      ...
                      {/* 모달 닫기 버튼 클릭 시 모달을 닫는 동시에 카메라 켜기*/}
                      <TouchableRipple style={styles.button} onPress={() => { setModalVisible(false); setUseCamera(true); }} accessible={true} accessibilityLabel='닫기' accessibilityRole='button' accessibilityHint='바코드를 인식하러 돌아갑니다'>
                        <Icon name="times" style={styles.Icon} color='black' size={50} accessibilityLabel='닫기' accessibilityRole='button' importantForAccessibility='no-hide-descendants'/>
                      </TouchableRipple>
